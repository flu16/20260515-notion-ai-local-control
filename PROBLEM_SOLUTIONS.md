...
输入验证的第一优先级不是逐字符一致，而是确认不会把旧输入框残留一起发送。
...
6. 粘贴后优先 exact match；长文本允许基于长度的 soft pass。
7. 如果 before_text 不属于新问题本身，但最终 actual 中仍包含 before_text，判定有旧文本残留。
```
...
因此不应把 similarity 作为长文本成功与否的核心门槛；它更适合作为诊断字段。
...
因此最终校验必须能识别旧残留；实现上用“先粘贴唤醒编辑器，再全选覆盖粘贴”的笨办法，
