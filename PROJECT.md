...
macOS Accessibility 旧路线仍保留为 legacy/debug 能力。

当前阶段不是 MCP 化阶段。项目重点是先把底层 UI 自动化能力做清晰、稳定、可测试：

- 通过 CDP 定位 `https://www.notion.so/quick-search`
- 通过 renderer DOM 写入、提交、等待并复制 Notion AI 回复
- 在 CDP 端口不可用时直接重启 Notion 并带 `--remote-debugging-port=9222`
- 支持 Notion AI 可上传附件：图片、PDF、CSV、Markdown、纯文本
- 保留 AX 扫描、点击、输入与状态脚本作为 legacy/debug 工具
...
`notion-ai ask-ax` 才调用旧 macOS Accessibility/剪贴板流程。

主流 CDP 路线只接受 `https://www.notion.so/quick-search` target，不操作
`https://www.notion.so/ai` 主页面。CDP 不可用时默认自动重启 Notion 并开启
`9222` 调试端口；如果 quick-search 浮层没有出现，直接失败并提示用户打开浮层，
不会自动回退到 AX。
...
原因：项目希望依赖可解释、可控的 CDP DOM 或 AX/键盘路径，而不是系统坐标点击。
CDP 内部的 `Input.dispatchMouseEvent` 只用于 renderer 内部文件选择器流程，不移动真实鼠标。
...
CLI
  ...
  ask_cdp.py                     # 默认 ask/ask-cdp 的 CDP 参数解析与提问流程
  ask_and_copy_reply.py          # ask-ax legacy 参数解析与输出格式

Ask workflow
  beta_cdp_input.py              # CDP target、DOM 输入、提交、附件和复制底层能力
...
./venv/bin/notion-ai ask-cdp "1+1" --json
...
`notion-ai ask` 是 AI agent 和人工快速调用的正式入口，默认使用 CDP。
`ask-cdp` 与它等价；`ask-ax` 仅用于旧流程调试或人工回退。
...
默认提问入口。通过 Electron CDP 完成完整闭环：写入问题、提交、等待生成完成、
复制当前最新回复。
...
`--from-stdin` 只是问题来源；写入 Notion AI 输入框时，脚本通过 CDP 写入 quick-search DOM 输入框。
直接把问题作为命令行参数只适合人工临时调试；`--from-clipboard` 保留给人工调试和旧自动化兼容。

流程：

1. 确认或启动 `127.0.0.1:9222` CDP 端口
2. 定位唯一 `https://www.notion.so/quick-search` page target
3. 通过 DOM 清空并写入 `[contenteditable="true"][role="textbox"]`
4. 点击 `提交 AI 消息`
5. 等待生成完成并点击最新 `拷贝回复`
...
legacy AX 提问入口，由 `notion-ai ask-ax` 调用。保留用于调试旧 macOS
Accessibility/剪贴板流程，不是默认路径。
...
- `notion-ai ask` / `ask-cdp` 通过 CDP 写入、提交、等待、复制回复
- `beta-cdp-input --status` 可读取 quick-search 输入框状态
- `beta-cdp-input "..."` 可写入 quick-search 输入框
- `beta-cdp-input --clear` 可清空 quick-search 输入框
- `ask --attach-file` 通过 CDP file chooser/file input 路线上传支持的附件

### AX legacy/debug

- 打开或检查 Notion AI 窗口：`open_ai_window.py`
- 列出普通 AI 窗口元素：`search_element.py --list`
- 搜索或点击 AX 按钮：`search_element.py` / `click_element.py`
- 读取或写入 AX 输入框：`input_box.py`
- 监听焦点元素并观察 `AXTitle` / `AXDescription`：`watch_focus.py`

## 常用调试流程

### 打开 legacy AX 窗口并列出元素
