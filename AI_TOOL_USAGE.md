...
调用方不需要先手动清空 Notion AI 输入框。`notion-ai ask` 写入问题时会默认执行双粘贴覆盖：
...
这不是失败。`notion-ai ask` 会自动点击回到底部按钮，然后再复制回复。
...
通常不需要手动调用，因为 `notion-ai ask` 会自己确保窗口打开。
