...
AI 调用方无论问题简单还是复杂，都建议先把完整问题写进系统剪贴板，再使用
`--from-clipboard`。这能避免 shell 引号、换行、代码块、路径空格和 `$()` 等内容被命令行提前解析。
...
```bash
./venv/bin/python ask_and_copy_reply.py --from-clipboard --timeout 180 --json
```
...
```bash
./venv/bin/python ask_and_copy_reply.py --from-clipboard --new_conversation --timeout 180 --json
```
...
可选。从系统剪贴板读取问题文本。AI/自动化调用方默认应该使用它。
