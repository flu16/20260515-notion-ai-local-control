...
当前 Notion AI 支持上传图片、PDF、CSV、Markdown、纯文本。本项目在
`input_box.normalize_file_paths(...)` 中先按扩展名做本地校验，只允许常见图片扩展名、
`.pdf`、`.csv`、`.md`、`.markdown`、`.txt`，避免把 `.docx`、`.xlsx`、`.json`
等不支持文件交给 Notion UI 后再失败。
